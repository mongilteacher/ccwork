...
`tests_passed` 같은 판정 필드는 위 명령이 검증하지만, **`scenario_count`·`tests_written` 같은 숫자는 아무도 확인하지 않으면 그대로 통과한다.** 실제로 1단계가 `scenario_count: 32`로 보고했는데 문서에는 28건뿐이었던 적이 있다(다음 단계가 문서를 정본으로 삼아 바로잡았지만, 그건 운이었다).

산출물 문서에서 직접 센다 — 코드를 읽는 게 아니라 줄을 세는 것이라 격리는 유지된다:

```bash
# 시나리오 수 (1단계 후)
grep -cE '^- \[[ x]?\] ?\[(정상|경계|예외)\]|^- \[(정상|경계|예외)\]' docs/features/{기능}/issue-{N}.md

# AC 수 (0단계에서 이미 셌다면 그 값과 대조)
gh issue view $ARGUMENTS --repo mongilteacher/ccwork --json body -q .body | grep -c '^- \[ \] Given'
```

**차이가 나면 STOP하지 말고, 문서의 실제 개수를 정본으로 삼아 이후 단계에 전달하고 로그에 남긴다.** 숫자 오보고는 판정을 뒤집지 않기 때문이다 — 다만 그 subagent의 다른 보고도 느슨할 수 있다는 신호이므로, 같은 단계에서 `uncovered_ac`처럼 **판정에 쓰이는 필드가 함께 어긋나 있으면 그때는 STOP**한다.
